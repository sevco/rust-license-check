...
export async function run(actionInput: input.Input): Promise<void> {
    ...
    console.info(`looking for licenses ${actionInput.licenses}`);
    for (const dep of report) {
        console.debug(`checking ${JSON.stringify(dep)}`);
        actionInput.licenses.forEach(function (license) {
            if (dep.license?.includes(license)) {
                warnings.push(dep);
            }
        });
    }
    if (warnings.length === 0) {
        core.info('No license violations were found');
    } else {
        core.warning(`${warnings.length} license violations found!`);
        shouldReport = true;
    }
